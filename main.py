...
class QuantumLottoKarmaApp(App):
    def build(self):
        ...
        for l_name in LOTTERIEN.keys():
            btn = ToggleButton(text=l_name, group="lotto", state="down" if l_name == "6aus49" else "normal", font_size=sp(9))
    ...
    def build_region_buttons(self, keys):
        ...
        for k in sorted(keys):
            btn = ToggleButton(text=k, group="region", state="down" if k == "BAYERN" else "normal", size_hint_y=None, height=dp(35), font_size=sp(10))
    ...
    def open_karma(self, instance):
        name, url = self.karma_manager.ziehe_projekt(self.current_region)
        webbrowser.open(url)
        self.status_label.text = f"Karma: {name}"
    ...
    def run_logic(self):
        ...
        if gezogene_z: res += f" | {config['zusatz']}: " + ", ".join(map(str, sorted(gezogene_z)))
    ...
    def add_to_history(self, text):
        self.history_list.add_widget(Label(text=text, font_size=sp(10), size_hint_y=None, height=dp(25)), index=0)
